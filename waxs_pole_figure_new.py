...
def compose_intensity(intensity, eta, omega, scans, peak, sample, debug_path):
    if not os.path.exists(debug_path):
        os.mkdir(debug_path)

    relative_intensity = np.zeros([scans, omega, eta])
    intensity_mat = np.zeros([scans, omega, eta])
    ...
    for i in range(scans):
        for j in range(omega):
            for k in range(eta):
                # print(index)
                intensity_mat[i, j, k] = intensity[index]
                index += 1
    
    for i in range(scans):
        for j in range(omega):
            for k in range(eta):             
                relative_intensity[i, j, k] = intensity_mat[i, j, k]/intensity_mat[0, 0, 0]
        # Code to save CSV files
        data_type = "intensity_" + str(i).rjust(2, '0') + "_"+ str(peak).rjust(2, '0')
        save_csv(debug_path, sample, data_type, relative_intensity[i, :, :])

    return relative_intensity

def compose_fwhm(fwhm, eta, omega, scans, peak, sample, debug_path):
    if not os.path.exists(debug_path):
        os.mkdir(debug_path)

    relative_fwhm = np.zeros([scans, omega, eta])
    fwhm_mat = np.zeros([scans, omega, eta])
    ...
    for i in range(scans):
        for j in range(omega):
            for k in range(eta):
                # print(index)
                fwhm_mat[i, j, k] = fwhm[index]
                index += 1
    
    for i in range(scans):
        for j in range(omega):
            for k in range(eta):             
                relative_fwhm[i, j, k] = fwhm_mat[i, j, k]/fwhm_mat[0, j, k]
        # Code to save CSV files
        data_type = "fwhm_" + str(i).rjust(2, '0') + "_"+ str(peak).rjust(2, '0')
        save_csv(debug_path, sample, data_type, relative_fwhm[i, :, :])

    return relative_fwhm

def make_plots(image_path, sample, type, scans, peak, data, lower_bound, upper_bound):
    ...
    for i in range(scans):
        ...
        ax.set_aspect('auto')
        path_comp = str(image_path) + sample + "_" + type + "_" + str(peak).rjust(2, '0') + "_" + str(i).rjust(2, '0')
...
def pole_figure_ti(type, lb, ub, peaks, sample, scans, data_path, image_path, debug_path):
    ...
    for peak in peaks:
        ...
        if type == "strain":
            ...
            make_plots(image_folder_path, sample, type, scans, peak, strain, lb, ub)
        elif type == "intensity":
            intensity = compose_intensity(
                parameter, eta, omega, scans, peak, sample, debug_folder_path)
            make_plots(image_folder_path, sample, type, scans, peak, intensity, lb, ub)
        elif type == "fwhm":
            fwhm = compose_fwhm(
                parameter, eta, omega, scans, peak, sample, debug_folder_path)
            make_plots(image_folder_path, sample, type, scans, peak, fwhm, lb, ub)
        else:
            print("error")
            quit()
...
def main():
    ...
    pole_figure_ti("strain", -.02, .02, np.arange(0, 6), "Ti-TiB-4v", 13, r"Y:\CHESS\ID3A_2022-2\lineouts\ti-tib-4v-eta-all\results\center",
                   r"Y:\CHESS\ID3A_2022-2\polefigures\ti-tib-4v", r"Y:\CHESS\ID3A_2022-2\debug\ti-tib-4v")
    pole_figure_ti("strain", -.02, .02, np.arange(0, 6), "Ti-TiB-4p", 38, r"Y:\CHESS\ID3A_2022-2\lineouts\ti-tib-4p-eta-all\results\center",
                   r"Y:\CHESS\ID3A_2022-2\polefigures\ti-tib-4p", r"Y:\CHESS\ID3A_2022-2\debug\ti-tib-4p")
    pole_figure_ti("strain", -.02, .02, np.arange(0, 6), "Ti-TiB-3", 15, r"Y:\CHESS\ID3A_2022-2\lineouts\ti-tib-3-eta-all\results\center",
                   r"Y:\CHESS\ID3A_2022-2\polefigures\ti-tib-3", r"Y:\CHESS\ID3A_2022-2\debug\ti-tib-3")
    pole_figure_ti("strain", -.02, .02, np.arange(0, 6), "Ti-TiB-2b", 27, r"Y:\CHESS\ID3A_2022-2\lineouts\ti-tib-2b-eta-all\results\center",
                   r"Y:\CHESS\ID3A_2022-2\polefigures\ti-tib-2b", r"Y:\CHESS\ID3A_2022-2\debug\ti-tib-2b")
    pole_figure_ti("strain", -.02, .02, np.arange(0, 6), "Ti-TiB-2", 24, r"Y:\CHESS\ID3A_2022-2\lineouts\ti-tib-2-eta-all\results\center",
                   r"Y:\CHESS\ID3A_2022-2\polefigures\ti-tib-2", r"Y:\CHESS\ID3A_2022-2\debug\ti-tib-2")
    pole_figure_ti("strain", -.02, .02, np.arange(0, 6), "Ti-TiB-1", 36, r"Y:\CHESS\ID3A_2022-2\lineouts\ti-tib-1-eta-all\results\center",
