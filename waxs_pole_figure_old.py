...
def extract_parameter(peak_index, data_path):
    ...
    for input_file_name in input_file_names:
        ...
        dot_index = input_file_name.rindex("-frame")
        csv_name = input_file_name[:dot_index] + "_"
        note = str('strain' + str(peak_index))
        counter = counter + 1
        ...
        #         strain = get_pole_figure_strain(sample, data_path, strain_to_calculate, omega, azm, peak_index)
        #         strain_data.append(strain)
...
def compose_strains(tth, eta, omega, sample, debug_path):

def pole_figure_ti(type, lb, ub, peaks, sample, data_path, debug_path, image_path):
    ...
    for peak in peaks:
        image_folder_path = ''.join([image_path, "\\", type, "_", str(peak), "\\"])
        parameter = extract_parameter(peak, data_path, debug_path, image_folder_path)

        if type == "strain":
            strain = compose_strains(parameter, eta, omega, sample, debug_path)
            (lb, ub, 
        # elif type == "intensity":
...
def main():
    ...
    pole_figure_ti("strain", -.003, .003, np.arange(0, 6), "Ti-TiB-1", r"Y:\CHESS\ID3A_2022-2\lineouts\ti-tib-1-eta-all\results\center",
                r"Y:\CHESS\ID3A_2022-2\polefigures\ti-tib-1", r"Y:\CHESS\ID3A_2022-2\debug\ti-tib-1")

    # # For APS 2021-3 Ni-CrC samples
