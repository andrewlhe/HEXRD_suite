...
#data = np.load('/nfs/chess/aux/user/lh644/ff_processing/cs-h-l1-s2_pole_test.npz')
data = np.load('/nfs/chess/aux/user/lh644/ff_processing/cs-n-l1-s2-pole2.npz')
...
for ii in np.arange(len(hkl_nums_use)):
    norm_i_list[:,ii]=i_list[:,ii]/np.max(i_list[:,ii]) #Normalize intensity

cm=plt.get_cmap('jet')

cnorm=colors.Normalize(vmin=0., vmax=1., clip=False)
scalarMap_i = cmap.ScalarMappable(norm=cnorm,cmap=cm)
scalarMap_i.set_array([0.,1.]) #set color map
...
for ii in range(len(n_s)):
    ax.scatter(n_s[ii,2,cur_hkl],n_s[ii,0,cur_hkl],n_s[ii,1,cur_hkl], c=color_val[ii,:],marker='o',s=50,edgecolor='none') #plot intensity pole figure


ax.scatter(0.,0.,0.,c=[0.8,0.8,0.8,1.0],marker='o',s=10000,edgecolor='none') #plot the sphere
...
plt.savefig(output_folder + '%s_Tlayer_1_ipf_load_%0.02d_hkl_%0.02d.png' %(file_header,scan_no,cur_hkl), bbox_inches='tight') #save figure
...
for ii in range(len(n_s)):
    ax.scatter(n_s[ii,2,cur_hkl],n_s[ii,0,cur_hkl],n_s[ii,1,cur_hkl], c=color_val[ii,:],marker='o',s=40,edgecolor='none')
...
plt.savefig(output_folder + '%s_Tlayer_1_spf_load_%0.02d_hkl_%0.02d.png' %(file_header,scan_no,cur_hkl), bbox_inches='tight')
